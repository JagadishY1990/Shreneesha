	...
	<script type="text/javascript">
		...
		$(function(){ // jQuery for handling onclick event
			document.getElementById('addLink').onclick =
					function () {
						//alert($("#addLink").href);
						document.getElementById('addLink').href = '<%=request.getContextPath()%>/edit/0';
					};
		});
	</script>
	...
	<div class="row">
		...
		<div class="container">
			...
			<div class="container text-left">
				...
				<a href="<%=request.getContextPath()%>/employees" id="" class="btn btn-success">List
					All Employees</a>
			</div>
			</c:if>
			<c:if test="${listEmployee != null}">
				...
				<tbody>
					...
					<c:forEach var="employee" items="${listEmployee}">
						...
						<tr>
							...
							<td><c:out value="${employee.yearsofexperience}" /></td>
							<td><c:out value="${employee.qualification}" /></td>
							<td><c:out value="${employee.certification}" /></td>
							<td><c:out value="${employee.technicalskills}" /></td>
							<td><a href="/employees/<c:out value='${employee.employeeID}' />">Edit</a>
								&nbsp;&nbsp;&nbsp;&nbsp; <a
								href="/deleteEmployee/<c:out value='${employee.employeeID}' />">Delete</a></td>
						</tr>
					</c:forEach>
